...
{
	m_ACoordinates = new Coordinates(x, y);
	m_BCoordinates = new Coordinates(x + width, y + height);
	m_Width = width;
	m_Height = height;
...
{
	m_ACoordinates->X += offset->X;
	m_ACoordinates->Y += offset->Y;
	m_BCoordinates->X += offset->X;
	m_BCoordinates->Y += offset->Y;
}
